...
class GPTBERT(GPTBERTPreTrainedModel):
    ...
    def get_contextualized_embeddings(
        ...
        elif attention_mask.dtype == torch.bool and len(attention_mask.size()) == 3:
        ...
        if causal_attention_mask is None:
            causal_attention_mask = self.is_causal

        causal_mask = input_ids.new_ones((seq_length, seq_length), dtype=torch.bool).triu(1)
        if isinstance(causal_attention_mask, torch.Tensor):
            causal_rows = causal_attention_mask.to(device=input_ids.device, dtype=torch.bool).view(batch_size, 1, 1, 1)
            attention_mask = attention_mask | (causal_rows & causal_mask.unsqueeze(0).unsqueeze(0))
        elif causal_attention_mask:
...
class GPTBERTForCausalLM(GPTBERTPreTrainedModel):
    ...
    _tied_weights_keys = {"lm_head.nonlinearity.5.weight": "model.embedding.word_embedding.weight"}
    ...
    def __init__(self, config, **kwargs):
        ...
        if not self.all_tied_weights_keys:
            self.all_tied_weights_keys = self._tied_weights_keys
    ...
    def forward(
        ...
        **kwargs
        ...
        sequence_output, contextualized_embeddings, attention_probs = self.model.get_contextualized_embeddings(
            input_ids,
            attention_mask,
            causal_attention_mask=causal_attention_mask,
        )
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # GPTBERT does not implement KV-cache. Always feed the full prefix back
        # during generation; otherwise Transformers may slice by cache_position
        # and produce out-of-range gather indices for this custom model.
